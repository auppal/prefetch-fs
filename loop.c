/*
 * Prefetching file system in userspace.
...
{
	cache_purge(ops, &cached_bytes, 0);
	return;
}
		    ...
		    struct fuse_file_info *fi)
{
	...
	struct loop_file_info *info
		= (struct loop_file_info *) (uintptr_t) fi->fh;
...
#if DEBUG
		fprintf(fp_sync, "cnt %d read %s fd %d inode %ju size %zu off %jd\n", cnt, path, fd, (intmax_t) ino, size, (intmax_t) offset);
		fflush(fp_sync);
#endif
...
#if 0
	off_t prefetch_stop_offset = prefetch_start_offset + prefetch_aggressiveness * prefetch_block_size;
#else
